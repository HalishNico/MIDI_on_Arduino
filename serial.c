...
int switch_arg(char opt, char* optarg);
...
int main(int argc, char *argv[]) 
{
    char optesque[64];
    char arg_command[64];
    memset(optesque, 0, sizeof(optesque));
    ...
    while(1) {
            ...
            scanf("%63s", optesque);
            for(int i=0, n=sizeof(optesque)/sizeof(optesque[0]);i<n;i++){
                if(optesque[i] == '-'){
                    i++;
                    opt = optesque[i];
                    i++;
                    int o=i;
                    while(optesque[i] != '-' && i<n){
                        arg_command[i-o]=optesque[i]; 
                        i++;
                    }
                    switch_arg(opt, arg_command);
                } else{
                   i++; 
                }
            }
...
int switch_arg(char opt, char* optarg){
        ...
        case 'd':
            ...
            sleep(n * 1000 ); // sleep milliseconds
            break;
        case 'h':
            usage();
            break;
...
int serialport_read_until(int fd, char* buf, char until)
{
    ...
    do { 
        ...
        if( n==0 ) {
            sleep( 10 * 1000 ); // wait 10 msec try again
            continue;
        }
        buf[i] = b[0]; i++;
    } while( b[0] != until );
